...
이번 블로그에서는 로그수집과 관제알림을 위해서 구축된 관제시스템 "Heimdall"에 대해서 공유하고자 합니다.
```

# 1. Heimdall 개요
---
## 1.1. Heimdall이란?
> 사람인HR에서 사용하던 기존 Argus 관제시스템에서 UI 및 기능을 개선한 새로운 관제시스템입니다. 
...
ex) 에러 로그 발생 또는 Excpetion 발생 시 관제 메시지 전송   
...
Kibana Dashboard에 로그 수집과 Metric에 대한 화면도 추가로 구성하였습니다. 
...
| 컨테이너 기반 로그 수집 |
|![Heimdall Container 아키텍쳐]({{site.url}}/img/heimdall/arch_container.png)|
| Baremetal 기반 로그 수집 |
|![Heimdall Bere 아키텍쳐]({{site.url}}/img/heimdall/arch_beremetal.png)|


## 2.2. Heimdall 구성 설명

### <span style="color:#4c80d9">관리자 화면</span>

- Heimdall Admin - 로그수집 설정 및 관제에 대한 설정을 관리할 수 있는 페이지입니다.

### <span style="color:#4c80d9">로그 설정</span>

- SideCar - 컨테이너 기반이 아닌 로그수집에 대해서 Filebeat/Metricbeat 설정을 변경할 수 있는 Agent입니다.
- Zookeeper - Filebeat/Metricbeat 설정 및 변경에 대한 Watcher 기능을 제공하는 저장소로 사용합니다.

### <span style="color:#4c80d9">로그수집 Agent</span>

- Filebeat - 일반적인 로그 수집을 위한 Agent입니다.
- Metricbeat - Metric 정보 수집을 위한 Agent입니다.
- Kafka - Filebeat에서 수집된 로그 데이터의 중간 저장소입니다.
- Logstash - Kafka에서 저장된 로그데이터를 가져와서 로그 패턴에 따라 분리 후 ElasticSearch에 저장해주는 역활을 합니다.

### <span style="color:#4c80d9">로그 수집저장소</span>

- ElasticSearch - 로그 및 Metric 데이터를 저장하는 저장소입니다.

### <span style="color:#4c80d9">DashBoard</span>

- Kibana - ElasticSearch 저장된 데이터를 기반으로 데이터 검색 및 Dashboard 및 그래프등을 표현해줍니다.

### <span style="color:#4c80d9">관제 알림</span>

- ElastAlert - ElasticSearch 저장된 데이터를 기반으로 Telegram으로 관제 메시지를 생성하여 전송해줍니다.  
...
<b style="color:yellow">어떻게 확인하게 될까요?</b>
	1. 어플리케이션이 6대의 서버에 설치되어 동작되고 있다면 모든 서버에 접근해야 할 것입니다.
	2. 서버에 접근 후 로그파일이 위치한 디렉토리에 이동 후 모든 로그파일 열고
	3. 어떤 서버에서 문제가 있는지 VI 검색이나 OS 명령어를 통해서 에러가 발생된 로그를 찾아야 할 것입니다.

<b style="color:yellow">위에 과정이 번거롭다고 느껴지지는 않나요?
어떻게 이 과정을 쉽게 할 수 있을까요?</b>
	Heimdall을 사용하면 서버에 직접 접근하지 않아도 특정 로그를 수집할 수 있습니다.
	6대의 서버에서 수집된 로그는 ElasticSearch에 날짜별로 특정 인덱스에 통합하여 저장되면
	Kibana를 사용하여 모든 로그를 한 곳에서 검색이 될 수 있습니다.
	또 이 로그들을 각각의 수치/그래프/DataTable 등 다양한 형태로 표현하고 Dashboard를 생성하면 한 곳에서
	한 눈에 원하는 정보를 확인할 수 있습니다.

<b style="color:yellow">어떤가요? 이제 관심이 좀 생기시나요? 아래 사용 사례를 정리해놨습니다.</b>
</pre>
...
```
STEP1. 설치 파일 다운로드
$ wget http://nexus.saraminhr.co.kr/content/repositories/releases/in/saram/heimdall.sidecar/0.0.2/heimdall.sidecar-0.0.2.tar.gz
$ wget http://nexus.saraminhr.co.kr/content/repositories/releases/in/saram/heimdall.sidecar/install-systemctl/heimdall.sidecar-install-systemctl.sh
$ wget http://nexus.saraminhr.co.kr/content/repositories/releases/in/saram/heimdall.sidecar/install-init/heimdall.sidecar-install-init.sh
...
| STEP1. 로그 수집 서버 지정 | 
| ![Sidecar Agent 지정]({{site.url}}/img/heimdall/heimdall_3_1.png){: width="800" height="400"} |
| STEP2. 로그 수집 설정 - 수집할 로그 파일 경로를 지정한다. |
|![Filebeat 수집 설정]({{site.url}}/img/heimdall/heimdall_3_2.png){: width="800" height="400"} |
...
input {
	kafka {
		bootstrap_servers => "heimdall.prod.kafka.saramin.host:31090,heimdall.prod.kafka.saramin.host:31091,heimdall.prod.kafka.saramin.host:31092"
		topics => ["lets"]
		codec => "json"
		consumer_threads => 3
	}                                                 
...
output {
	elasticsearch {
		hosts => ["elasticsearch.heimdall:9200"]
...
![Kibana 검색 결과]({{site.url}}/img/heimdall/heimdall_3_3.png)
...
![Lets Dashboard 화면]({{site.url}}/img/heimdall/heimdall_3_4.png)
...
| STEP1. 관제 Rule 추가 | 
| ![관제룰 등록]({{site.url}}/img/heimdall/heimdall_3_7.png){: width="800" height="400"} |
| STEP2. 관제 추가 후 목록에서 관제 기능 ON/OFF 지정도 가능 |
| ![관제 목록]({{site.url}}/img/heimdall/heimdall_3_8.png){: width="800" height="400"} |
| STEP3. 관제 발생 시 Telegram으로 메시지 전송 |
| ![ElastAlert]({{site.url}}/img/heimdall/heimdall_7.png){: width="800" height="400"} |



## 3.2. Metric 수집을 활용한 Kafka 상태 정보 확인
<pre style="color:yellow; background-color:black">
서버에 어플리케이션에 연동된 DB, Kafka, Redis가 갑자기 중지되어 장애가 난 경험을 한 적 없으신가요?
Heimdall에서는 이런 연동 모듈에 대해서 상태 정보를 수집하여 다양한 정보를 제공합니다.
</pre>
...
|![Metric 수집 설정]({{site.url}}/img/heimdall/heimdall_3_5.png){: width="800" height="400"} |
...
![Metric 검색 결과]({{site.url}}/img/heimdall/heimdall_3_6.png)
...
Kafka Dashboard 구성 예제입니다.
	...
	Consumer Group Lag by Topic : Topic에 데이터가 쌓이게 되면 Lag 수치가 올라가는데 이 수치를 그래프로 표시
..생략.. 
``` 

![Kafka 대쉬보드]({{site.url}}/img/heimdall/heimdall_10.png)
...
| 기존 관리자 화면 | 
|![Heimdall 기존 관리자 화면]({{site.url}}/img/heimdall/heimdall_4_1.png){: width="800" height="300"} |
| 신규 관리자 화면| 
|![Heimdall 신규 관리자 화면]({{site.url}}/img/heimdall/heimdall_2.png){: width="800" height="300"} |
...
variables:
  CI_REGISTRY: sri-docker-registry.saraminhr.co.kr:5000/platform/heimdall-admin
...
build admin:
  ...
  script:
    - >  # admin build
      ...
      -f heimdall.admin/build/resources/main/Dockerfile
...
![Rancher 배포 화면]({{site.url}}/img/heimdall/heimdall_3.png)


4) Admin이 배포되어 기동된 것을 확인할 수 있습니다.
![Kubernetes Deployment]({{site.url}}/img/heimdall/heimdall_4.png)
...
문제점1. Logstash 지연 및 성능 이슈
	- 로그에 대한 수집과 로그 문자열 분리 작업을 모두 처리하게 되어서 부하가 발생되고
	- 이로 인하여 성능 저하와 로그 유실이 발생되었습니다.
문제점2. 로그 파일 관리 문제
	- Logstash 설정 파일 하나에서 여러개의 로그 파일을 관리하기 때문에
	- 로그파일이 증가될 때마다 내용도 길어지고, 조건식 분기 구문이 늘어나는 구조여서 관리가 힘들었습니다.
```
...
개선1. 로그 수집 분리
	...
	- Filebeat과 Logstash 사이에 중간 버퍼 역활을 위한 Kafka를 구성하여 Logstash의 부하를 줄이므로써
	- 로그 지연 및 성능을 개선하였습니다.
개선2. 로그 파일 관리 개선
	- Filebeat에서 로그 개수에 따라 설정 파일을 분리해서 로그 관리가 수월해졌습니다.	
```
...
![Filebeat 컨테이너 흐름]({{site.url}}/img/heimdall/heimdall_4_4.png)


1-1) Rancher에서 설정
* Rancher + Helm Chart를 사용하여 컨테이너 로그 수집에 대한 ON/OFF를 설정하게 했습니다.

![Rancher Sidecar]({{site.url}}/img/heimdall/heimdall_4_2.png)
...
- type: log
  paths:
    - '/home/platform/heimdall.admin/data/logs/heimdall-webapp.log'
  exclude_lines: ['DEBUG']
  tail_files: "true"
  ## json.message_key: log
  ## json.keys_under_root: false
...
어플리케이션에서 연동되는 서버스가 문제가 되는 경우, 장애에 대한 인지가 늦어 대응이 늦는 경우가 발생되었습니다.
이에 대응하기 위해서 <b style="color:yellow">Metric 기반의 로그를 수집하여 정상 기동 및 장애 발생에 대응할 수 있었습니다.</b>
다른 한가지 이유는 Heimdall 시스템 및 기존 어플리케이션을 Kubernetes 환경에서 배포가 진행되면서 
Kubernetes에 대한 서비스, 노드, 파드에 대한 관리가 필요하기 때문에 Metric 기반 로그 수집은 중요해졌습니다.
...
![Metric 로그 수집 Flow]({{site.url}}/img/heimdall/heimdall_4_5.png)
...
```SATA
Metric 로그 수집도 K8S 컨테이너 기반의 로그수집과 Baremetal Server 기반의 기존 로그수집 모두 가능합니다.

1. K8S 컨테이너 기반의 로그수집	
	Helm Chart로 로그 수집 설정이 관리되면 Rancher에서 ON/OFF를 지정할 수 있습니다.  
	로그수집을 ON으로 지정 시 컨테이너 내부에 Metricbeat Sidecar 형식으로 동작됩니다.

2. Baremetal Server 기반의 기존 로그수집
	Heimdall Admin에서 Metric 설정 페이지가 추가되었습니다.
	Metric 로그 수집을 설정하게 되면 지정된 서버에 설치된 Sidecar Agent를 통해서 Metricbeat 통해서 수집이 진행됩니다.
...
> 기존에는 any, frequency에 대한 관제 타입 설정만 가능했다.  
> 고도화를 진행하면 다양한 관제 타입을 추가하였다.    
...
> ElastAlert 구성 
![ElastAlert]({{site.url}}/img/heimdall/heimdall_7.png)
...
![Kibana]({{site.url}}/img/heimdall/heimdall_8.png)

### 4.5.1 로그 기반 대쉬보드 구성 예제
```SATA
아래 그림은 사람인HR IT연구소의 푸쉬 전송 시스템의 로그를 기반으로 대쉬보드를 구성한 경우입니다.
메시지 푸쉬 타입별 카운트를 DataTable/Line Graphe등으로 표시하고 총 전송개수 및 발송에 대한 수치는  Metric으로 표현했습니다. 
데이터 분석 시간은 분/시간/일/월 등으로 범위지정도 가능합니다. 
```

![어플리케이션 대쉬보드]({{site.url}}/img/heimdall/heimdall_9.png)


### 4.5.2 Metric 기반 대쉬보드 구성 예제
```SATA
Metric 로그 수집이 추가되면서 Kibana에서 연동 시스템에 대한 정보도 대쉬보드를 추가하였습니다.
...
![Metric 대쉬보드]({{site.url}}/img/heimdall/heimdall_10.png)
...
서비스인프라개발팀에서는 ELK 기반에서 로그에 대한 처리를 조금 더 쉽게 처리할 수 있게 고민하여 
지금의 시스템이 구축하였습니다.  
